...
func _refresh_from_frame() -> void:
	...
	var map_context := _map_renderer.refresh_map(map)
	_map_renderer.refresh_objects(objects)
	if !map_context.is_empty():
		_camera_controller.apply_map_context(map_context)
	...
	for unit in units:
		...
		_selection.track_unit(unit_id, unit)
		_unit_renderer.update_unit(unit_id, unit, _selection.selected_unit_id)

	var removed_unit_ids := _unit_renderer.remove_missing_units(alive_unit_ids)
	_selection.remove_units(removed_unit_ids)
